#= Programming =#
# result[1] = f0(input_seq[1]), result[n] = cf(result[n-1], input_seq[n])
function chain_reduce(f0, cf, input_seq)
    len = length(input_seq)
    if len == 0 return []
    else
        seed = f0(input_seq[1])
        result = [seed]
        for el in drop(input_seq, 1)
            seed = cf(seed, el)
            push!(result, seed)
        end
    end
...
# result[1] = f0(input_seq[1]), result[n] = cf(input_seq[n-1], input_seq[n])
function chain_map(f0, cf, input_seq)
    len = length(input_seq)
    if len == 0 return []
    else
        seed = f0(input_seq[1])
        ...
        for i in 2:length(input_seq)
            seed = cf(input_seq[i-1], input_seq[i])
            push!(result, seed)
        end
    end
    return result
...
function sample_columns(X, fraction, sampling = :random)
    xc = size(X, 2)
    if sampling == :random
        return [X[:, ci] for ci in rand(1:xc, round(Int, fraction*xc))]
    elseif sampling == :uniform
        return [X[:, ci] for ci in 1:round(Int, 1/fraction):xc]
    end
end
...
# :energy.
function basis_dist(est_basis::Array{Float, 2}, svd_basis::Array{Float, 2},
                    svalues::Array{Float, 1}, metric = :energy)
    metric_expts = Dict(:variance => 1, :energy => 2)
