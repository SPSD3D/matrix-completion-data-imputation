...
for (i in fnms){
  ...
  Result<-as.matrix(Imp)
  Result<-Result[1:nrow(Case),1:ncol(Case)]
  Xreal<-data.matrix(Data)
  Ximp <-data.matrix(Result)
  
  if(writecompleted){
    write.table(Ximp,file=paste(completedfolder,"completed_mice_",i,sep=""),row.names=FALSE,col.names=FALSE, sep=",")
  }
...
targetfilepath<-paste(resultsfolder,datasettarget,"_knn.csv",sep="")
