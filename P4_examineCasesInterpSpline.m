...
for k=1:length(H)
   ...
   x(y==0)=[];
   y(y==0)=[];
   Result = interp1(x,y,xq,'spline');
   Result=Result(2:end-1);
   xq=1:length(Result);
   GroundTruth=reshape(Data,1,[]);
   
   Result(nz)=GroundTruth(nz);
   
   if writecompleted
   completed=reshape(Result,size(Data));

   csvwrite([completedfolder  'completed_spline_' fnm],completed);
   end
...
csvwrite([resultsfolder  datasettarget '_spline.csv'],resultsCollection);